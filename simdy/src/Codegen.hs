...
import LLVM.Internal.FFI.BinaryOperator (hasNoSignedWrap)
...
fnPtr nm = gets (findType . moduleDefinitions)
  where
    findType defs =
      case fnDefByName of
...
iadd :: Type -> Operand -> Operand -> Codegen Operand
iadd t a b = instr t $ Add False False a b []

isub :: Type -> Operand -> Operand -> Codegen Operand
isub t a b = instr t $ Sub False False a b []

imul :: Type -> Operand -> Operand -> Codegen Operand
imul t a b = instr t $ Mul False False a b []

idiv :: Type -> Operand -> Operand -> Codegen Operand
idiv t a b = instr t $ SDiv False  a b []

icmp :: I.IntegerPredicate -> Type -> Operand -> Operand -> Codegen Operand
icmp cond t a b = instr t $ ICmp cond a b []
...
fptosi ty a = instr i32 $ FPToSI a ty []
