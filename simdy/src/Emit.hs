...
emit ::
       (MonadFix m, LLVM.IRBuilder.Monad.MonadIRBuilder m, MonadModuleBuilder m,
        MonadState NameMap m) =>
       Syn.Expr -> m AST.Operand
...
emit (Syn.While cond bodyBlock)
  = mdo
       ...
       whileBody <- buildBranch "whileBody" bodyBlock resultPointer $
                      Just whileStart
...
buildBranch ::
              (MonadIRBuilder m, MonadFix m, MonadModuleBuilder m,
               MonadState NameMap m) =>
              ShortByteString -> [Syn.Expr] -> p -> Maybe AST.Name -> m AST.Name
buildBranch name codeBlock resultPointer mNext
  = do branch <- block `named` name
...
buildCodeBlock ::
                 (MonadFix m, MonadIRBuilder m, MonadModuleBuilder m,
                  MonadState NameMap m) =>
                 [Syn.Expr] -> m AST.Operand
buildCodeBlock exprBlock
  -- Steps of codegen
  = do ops <- mapM emit exprBlock
       return (last ops)

funcBodyBuilder ::
                  (MonadIRBuilder m, MonadState NameMap m, MonadFix m,
                   MonadModuleBuilder m) =>
                  [Syn.Expr] -> [Syn.Expr] -> p -> m ()
funcBodyBuilder bodyTokens args = funcBody
  where funcBody argsOperands
          = do namedBlock <- block `named` bodyLabel
               allocArgs args
...
allocArgs (e@(Syn.DefVar varname vartype) : exprs)
  = do p <- allocateT vartype `named` toShort' varname
...
buildFunction ::
                (MonadFix m, MonadModuleBuilder m,
...
getVar ::
         (MonadState NameMap m, MonadModuleBuilder m, MonadIRBuilder m,
          MonadFix m) =>
         Syn.Expr -> String -> m AST.Operand
