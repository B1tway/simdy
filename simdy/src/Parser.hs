...
import Lexer
       (angles, braces, brackets, commaSep, decimal', identifier, int', lexer,
        parens, reserved, reservedOp)
import Syntax
...
binary ::
       String ->
         Op ->
           Ex.Assoc -> Ex.Operator String () Data.Functor.Identity.Identity Expr
binary s f = Ex.Infix (reservedOp s >> return (BinOp f))

prefix ::
       String ->
         UnaryOp -> Ex.Operator String () Data.Functor.Identity.Identity Expr
...
factor :: Parser Expr
factor
  = try idx <|> function <|> try variableDef <|> try cast <|> try load <|>
      try store
      <|> try decimal
      <|> try int
      <|> try call
...
parsePrimitive
  = do try variableI32 <|> try variableU32 <|> try variableI16 <|>
         try variableU16
         <|> try variableDouble
         <|> variableFloat
...
variableDef
  = do varName <- identifier
       reservedOp ":"
       varType <- try variablePtr <|> try variableVec <|>
...
cast
  = do reserved "bitcast"
       typeId <- angles $
                   try variablePtr <|> try variableVec <|>
                     (Primitive <$> parsePrimitive)
